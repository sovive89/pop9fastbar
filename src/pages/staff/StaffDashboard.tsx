...
const StaffDashboard = () => {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, () => { fetchAll(); fetchRecentOrders(); })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items' }, () => { fetchAll(); fetchRecentOrders(); })
      .subscribe();
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {sessions.map((session, i) => {
                  ...
                  return (
                    <div
                      key={session.id}
                      className="glass rounded-2xl border-2 border-primary/40 overflow-hidden animate-slide-up cursor-pointer group hover:border-primary/60 transition-all"
                      style={{ animationDelay: `${i * 0.03}s` }}
                    >
                      {/* Main cell */}
                      <div className="p-3 text-center">
                        <p className="font-display font-bold text-2xl text-foreground leading-none">{code}</p>
                        ...
                        {session.clients?.map(client => (
                          <div key={client.id} className="flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="text-[9px] text-primary hover:text-accent flex items-center gap-0.5"
                            >
                      ...
                      <div className="border-t border-border/20 flex">
                        <button
                          ...
                          className="flex-1 py-1.5 text-[10px] text-muted-foreground hover:text-primary flex items-center justify-center gap-0.5 transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 py-1.5 text-[10px] text-destructive/70 hover:text-destructive flex items-center justify-center gap-0.5 transition-colors"
                        >
                          <XCircle className="w-2.5 h-2.5" /> Fechar
                        </button>
                      </div>
                    </div>
                  );
